...
func (a *AppHandler) MarkMessageAsRead(w http.ResponseWriter, r *http.Request, token string, parts []string) {
	...
	err2 := database.MarkMessageAsRead(a.DB, int64(messageid), int64(userid), int64(chatid), isGroup)
	if err2 != nil {
		http.Error(w, "message not found or not authorized to mark as read", http.StatusNotFound)
		return
	}
