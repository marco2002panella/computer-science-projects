...
func (a *AppHandler) deleteComment(w http.ResponseWriter, r *http.Request, token string, parts []string) {
	if token != parts[1] {
	...
	reactionid, err5 := strconv.Atoi(parts[7])
	if err5 != nil {
		http.Error(w, "id not valid", http.StatusBadRequest)
		return
	}
	...
	//basically controlla se l'utente è membro della chat, se non lo è non può eliminare la reazione (controllo di sicurezza)
	...
	err2 := database.DeleteReaction(a.DB, int64(messageid), int64(userid), int64(reactionid), isGroup, int64(chatid))
	if err2 != nil {
		http.Error(w, "message or reaction not found or not authorized to delete", http.StatusNotFound)
		return
	}
